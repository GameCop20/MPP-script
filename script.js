(function () {
  try {
    // === variables ===
    const devs = ["4e9d7ca429fcfa460efff99a", "97f34e0fdf4a942adae2e2af"];
    ...
    MPP.client.on("a", (msg) => {
      ...
      if (cmd === `${p}help`) {
        if (!input) {
          if (lang === `en`)
            return sendChat(
              `Please choose category - Main | Fun | Dev.`,
              msg.id
            );
          else if (lang === `ru`)
            return sendChat(
              `Пожалуйста выберите категорию - Главная | Веселье | Разработка`,
              msg.id
            );
